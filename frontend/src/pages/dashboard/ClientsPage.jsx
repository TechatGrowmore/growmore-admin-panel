...
export default function ClientsPage() {
  ...
  const handleTestConnection = async (client) => {
    ...
    try {
      const res = await fetch(`/api/proxy/${client.id}/health`, { credentials: 'include' });
      const data = await res.json();
      setTestResults((prev) => ({
        ...prev,
        [client.id]: res.ok
      ...
      setTestResults((prev) => ({
        ...prev,
        [client.id]: { success: false, error: err.message },
      }));
    } finally {
      setTesting(null);
    }
      ...
      >
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.87rem' }}>
            Connect and manage your valet parking client apps
          </p>
        </div>
        <button
          id="add-client-btn"
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  placeholder="e.g. https://bennecafevaletapp.onrender.com"
                />
              </div>
              <div className="form-group">
                <label>

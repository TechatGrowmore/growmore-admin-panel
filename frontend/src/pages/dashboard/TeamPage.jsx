...
export default function TeamPage() {
  ...
  const fetchTeam = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/proxy/${selectedClient}/users`, { credentials: 'include' });
  ...
  return (
    <div className="page-body">
      ...
      >
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.87rem' }}>
            Drivers, supervisors, and managers per venue
          </p>
        </div>
      ...
      {loading ? (
        <div className="page-loading"><div className="spinner" /><span>Loading staff list...</span></div>
      ) : !teamData ? (
        ...
        <>
          ...
          <div className="chart-card" style={{ marginBottom: 20 }}>
            <div className="chart-tabs">
              {[
                { key: 'all', label: `All Staff (${drivers.length + supervisors.length + managers.length})` },
                { key: 'driver', label: `Drivers (${drivers.length})` },
                { key: 'supervisor', label: `Supervisors (${supervisors.length})` },
              ].map(({ key, label }) => (
                <button
                  key={key}
                  className={`chart-tab ${roleFilter === key ? 'active' : ''}`}
          ...
          <div className="data-table-container">
            <table className="data-table">
              <thead>
                <tr>
                  <th>Name</th><th>Role</th><th>Phone</th><th>Assigned Venue</th>
                  <th>Supervisor / Manager</th><th>Today&apos;s Bookings</th><th>Status</th>
                </tr>
              </thead>
              <tbody>

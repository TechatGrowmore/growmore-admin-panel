...
export default function DashboardPage() {
    ...
    try {
      ...
      await Promise.allSettled(
        (data.clients || []).map(async (client) => {
          try {
            const sRes = await fetch(`/api/proxy/${client.id}/summary`);
            if (sRes.ok) {
  ...
  const totals = Object.values(clientData).reduce((acc, d) => {
    ...
    <div className="page-body">
      ...
      <div className="stats-grid">
        <div className="stat-card" style={{ '--card-accent': '#FF6B35' }}>
          ...
          <div className="stat-card-sub">across {clients.length} client{clients.length !== 1 ? 's' : ''}</div>
        </div>
              ...
              <div key={client.id} className="client-card">
                ...
                {hasData ? (
                  <div className="client-card-stats">
                    ...
                    <div className="client-stat">
                      <div className="client-stat-value">₹{(data.today?.revenue || 0).toLocaleString('en-IN')}</div>
                    ...
                    <div className="client-stat">
                      <div className="client-stat-value">₹{(data.allTime?.revenue || 0).toLocaleString('en-IN')}</div>

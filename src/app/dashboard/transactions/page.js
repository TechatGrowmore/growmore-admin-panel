...
export default function TransactionsPage() {
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/proxy/${selectedClient}/bookings?${params.toString()}`);
  ...
  // Filter completed/paid transactions only
    ...
    <div className="page-body">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.87rem' }}>Day-wise payment history and revenue collections</p>
        </div>
      ...
      <div className="stat-card" style={{ '--card-accent': '#10B981', marginBottom: 24 }}>
        ...
        <div className="stat-card-sub">{paidTransactions.length} successful transaction{paidTransactions.length !== 1 ? 's' : ''}</div>
      </div>
              ...
              <tbody>
                {group.items.map((b) => (
                  <tr key={b._id || b.bookingId}>
                    ...
                    <td>
                      <strong style={{ color: 'var(--accent)' }}>{b.vehicle?.number}</strong>
                    </td>
